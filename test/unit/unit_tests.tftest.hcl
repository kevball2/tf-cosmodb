...
run "test_resource_group_name_validation_succeeds" {
  command = plan
  variables {
    resource_group_name = "badresourcegroupname."
  }
...
run "test_lock_name_not_provided_value" {
  command = plan
  assert {
    condition     = azurerm_management_lock.this[0].name == "lock-${var.name}"
...
run "test_role_assignment_role_assignments" {
  command = plan
  variables {
    role_assignments = {
...
run "test_role_assignments_role_definition_id" {
  command = plan
  variables {
    role_assignments = {
...
run "test_role_assignments_role_definition_id_validation_succeeds" {
  command = plan
  variables {
    role_assignments = {
      "role_assignment_id_test" = {
